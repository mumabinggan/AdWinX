//
//  ADAdWinXInterstitialAdDetailViewController.m
//  AdWinX
//
//  Created by AdWinX on 08/31/2026.
//

#import "ADAdWinXInterstitialAdDetailViewController.h"
#import <AdWinX/AdWinX.h>

@interface ADAdWinXInterstitialAdDetailViewController ()
...
@implementation ADAdWinXInterstitialAdDetailViewController

- (void)viewDidLoad
{
    [super viewDidLoad];

    // 演示预载生命周期：进入页面（≈关卡快结束）即后台预载，触发点直接取走展示
    __weak typeof(self) weakSelf = self;
    [[ADXAdManager sharedManager] preloadInterstitialAdWithSlotName:self.placementId
...
- (void)loadAd
{
    [self updateStatus:@"AdWinX 聚合插屏拍卖中（GDT + 百度 竞价，Sigmob + 穿山甲 瀑布）..." loading:YES];
    __weak typeof(self) weakSelf = self;
    [[ADXAdManager sharedManager] loadInterstitialAdWithSlotName:self.placementId
                                                      completion:^(ADXAuctionResult *result) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        ...
        if (result.winnerResult) {
            ...
            [strongSelf.showAdButton setTitle:@"展示插屏广告" forState:UIControlStateNormal];
        } else {
...
- (void)showAdButtonTapped
{
    ...
    ADXAuctionResult *preloaded = [[ADXAdManager sharedManager] takeInterstitialAdWithSlotName:self.placementId];
...
- (void)showCurrentResult
{
    ...
    __weak typeof(self) weakSelf = self;
    [[ADXAdManager sharedManager] showInterstitialAdWithResult:self.auctionResult
        ...
        // 展示结束补一次预载（下次触发点还有广告可用）
        [[ADXAdManager sharedManager] preloadInterstitialAdWithSlotName:strongSelf.placementId completion:nil];
    }];
}
